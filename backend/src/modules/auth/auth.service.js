...
class AuthService {
    async register({ first_name, last_name, email, phone, password }) {
        ...
        await brevo.sendOtp(email, otp.code, 'email_verify');
    ...
    async verifyEmail({ email, code }) {
        ...
        user.status = 'active';
        await user.save();
        brevo.sendWelcome(user.email, user.first_name).catch(() => { });
        return user;
    }
    ...
    async resendOtp({ email, purpose }) {
        ...
        await brevo.sendOtp(email, otp.code, purpose);
        return otp.code;
    }

    async login({ email, password }, meta = {}) {
        await security.assertLoginAllowed(email);
    ...
    async refresh({ refresh_token }, meta = {}) {
        ...
        const stored = await RefreshToken.findOne({ where: { token: tokenHash, user_id: payload.id } });
    ...
    async logout({ refresh_token, userId }) {
        ...
        if (refresh_token) where.token = hashToken(refresh_token);
    ...
    async forgotPassword({ email }) {
        ...
        await brevo.sendOtp(email, otp.code, 'password_reset');
        return otp.code;
    }
    ...
    async #issueTokens(user, meta) {
        ...
        await RefreshToken.create({
            user_id: user.id,
            token: hashToken(refreshToken),    // <-- store HASH only
    ...
    async #createOtp(userId, email, purpose) {
         await security.assertOtpAllowed(email, purpose);
