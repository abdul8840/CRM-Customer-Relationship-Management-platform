...
export default function Billing() {
  ...
  const subscribe = async (plan) => {
    try {
      const { order, key_id, free, sub } = await subscriptionApi.checkout(plan.id);
      if (free) { toast.success(`Activated ${plan.name}`); qc.invalidateQueries({ queryKey: ['my-sub'] }); return; }
      const rzp = new window.Razorpay({
        key: key_id, amount: order.amount, currency: order.currency, order_id: order.id,
        name: 'CRM Platform', description: `${plan.name} subscription`,
        prefill: { email: user?.email, name: `${user?.first_name} ${user?.last_name}`, contact: user?.phone },
        theme: { color: '#2563eb' },
        handler: async (res) => {
          try {
            await subscriptionApi.verify({
              order_id: res.razorpay_order_id, payment_id: res.razorpay_payment_id,
              signature: res.razorpay_signature, plan_id: plan.id,
            });
